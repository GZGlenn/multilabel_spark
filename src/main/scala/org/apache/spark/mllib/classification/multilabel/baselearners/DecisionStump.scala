...
case class DecisionStumpModel(
  ...
  override def predict(features: Vector): Vector = {
    Vectors.fromBreeze(votes.asBreeze * alpha * {
      cut match {
...
class DecisionStumpAlgorithm(
  ...
  override def run(dataSet: RDD[WeightedMultiLabeledPoint]): DecisionStumpModel = {
    ...
    val parLen = SparkContext.getOrCreate().broadcast(dataSet.partitions.length)

//    println(parLen.value)

    val groupedDataSet = dataSet
      .map(info => (info.hashCode() % (parLen.value * 2), info))
      .groupByKey()
      .map(_._2.toArray)
...
object DecisionStumpAlgorithm {
  ...
  def getLocalSplitMetrics(featureSet: Iterator[Int])(
    ...
    val numClasses = dataSet(0).data.labels.size
    // 1. initial edge
...
//    println(initialEdge.toString)
  ...
  def aggregateSplitMetrics(allSplitMetrics: RDD[SplitMetric]): RDD[SplitMetric] = {
    ...
    allSplitMetrics.map {
      ...
      SplitMetric(metric1.featureCut, Vectors.fromBreeze(metric1.edges.asBreeze + metric2.edges.asBreeze))
